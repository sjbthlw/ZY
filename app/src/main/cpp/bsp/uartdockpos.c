...
static char *dev = "/dev/ttysWK0";    //232串口(和打印小票的串口冲突)
...
u8 UartDockpos_Init(u8 BaudID) {
    ...
    //printf("ok:%s open ok\n",dev);
    LOGD("UartPrinter_Init BaudID:%d", BaudID);
    Uart_Init(uart_fd, 0, 'E');
    return 0;
}
...
u8 UartDockpos_SendData(u8 *data, u16 len) {
    u16 i;
    int ret;
    u8 cSendData[1024];
...
u8 UartDockpos_RecvData(u8 *data) {
    int ret;
    u32 iTimerCount = 0;
    u8 rcv_buf[256];
...
//        LOGD("UartPrinter_RecvData read ret:%d", ret);
        Sleepms(1);
        if (ret < 1) {
            continue;
        } else {
...
u8 UartDockpos_ClearData() {
    ...
    //LOGD("UartPrinter_ClearData:recvlen:%d",recvlen);
    if (recvlen != 0)
        return 0;
    else
        return 1;
