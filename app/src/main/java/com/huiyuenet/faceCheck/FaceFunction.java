...
//    public synchronized static float faceLiveCheck(byte[] pixelsBGR, int width, int height) {
//        float[] liveScore = new float[1];
//        if (pixelsBGR == null) {
//            return liveScore[0];
//        }
//        FaceCheck.DetectLive1(pixelsBGR, width, height, liveScore);
//        return liveScore[0];
//    }
//
//    public synchronized static float[] faceLiveCheck(byte[] pixelsBGR, byte[] pixelsIR, int width, int height,
//                                                     int threshold, THFI_FacePos[] facePosRgb, THFI_FacePos[] facePosIr) {
//        float[] liveScore = new float[2];
//        if (pixelsBGR == null && pixelsIR == null) {
//            return liveScore;
//        }
//        FaceCheck.DetectLive2(pixelsBGR, pixelsIR, facePosRgb, facePosIr, width, height, threshold, liveScore);
//        return liveScore;
//    }
...
//        return pixels;
        ...
        int w = bmp.getWidth();
        int h = bmp.getHeight();
//        Log.d(targer,"图片宽度w:"+w);
//        Log.d(targer,"图片高度h:"+h);
        int sizeBitmap = bmp.getByteCount();   //获取bitmap的大小 = w*h*4
//        Log.d(targer,"获取图片的大小:"+sizeBitmap);
        if(sizeBitmap> (2048*2048*4)){
            Log.e(targer,"获取图片的大小超出范围:"+sizeBitmap);
            return null;
        }
