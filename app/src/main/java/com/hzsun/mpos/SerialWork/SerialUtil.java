...
public class SerialUtil {
    ...
    public static long getOrdernum(byte[] bytes) {
        String ordernumStr = "";
        for (int i = 0; i < 6; i++) {
            int num = bytes[i] & 0xff;
            ordernumStr += String.format("%02d", num);
        }
        int order = 0;
        order += bytes[6] & 0xff;
        order += bytes[7] * 255;
        ordernumStr += String.format("%02d", order);
        if (TextUtils.isEmpty(ordernumStr)) {
            return 0;
        }
        long ordernum = 0;
        try {
            ordernum = Long.parseLong(ordernumStr);
        } catch (Exception e) {
            e.printStackTrace();
