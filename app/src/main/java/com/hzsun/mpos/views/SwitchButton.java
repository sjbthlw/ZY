...
public class SwitchButton extends View implements View.OnClickListener {
    ...
    private int color_switch_blue;
    private int color_switch_white;

    private void initData() {
        color_switch_blue = getResources().getColor(R.color.color_switch_blue);
        color_switch_white = getResources().getColor(R.color.colorWhite);
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        mSwitchBallx = isFirstChecked ? 90 : mStrokeRadius;
    ...
    public void setChecked(boolean isChecked) {
        mCurrentState = isChecked ? State.OPEN : State.CLOSE;
        if (mCurrentState == State.CLOSE) {
            mBallPaint = createPaint(color_switch_white, 0, Paint.Style.FILL, 0);
        } else {
            isFirstChecked = true;
            mBallPaint = createPaint(color_switch_blue, 0, Paint.Style.FILL, 0);
            mBgPaint.setColor(color_switch_white);
        }
    ...
    public void onClick(View v) {
        ...
        if (mCurrentState == State.CLOSE) {
            mBallPaint = createPaint(color_switch_white, 0, Paint.Style.FILL, 0);
            animate(BALL_X_RIGHT, mStrokeRadius, color_switch_white, color_switch_blue);
        } else {
            mBallPaint = createPaint(color_switch_blue, 0, Paint.Style.FILL, 0);
            animate(mStrokeRadius, BALL_X_RIGHT, color_switch_blue, color_switch_white);
        }
        if (mOnCheckedChangeListener != null) {
            if (mCurrentState == State.OPEN) {
                mOnCheckedChangeListener.onCheckedChanged(this, true);
