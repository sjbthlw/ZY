...
import static com.hzsun.mpos.Global.Global.BEYOND_DAY_TOTAL;
...
public class Consume {
    ...
    public static int Consume_CheckOut() {
        ...
        if (cResult == OK) {
            ...
            //Publicfun.PrintfAllCardInfo();//打印卡片基础信息
            g_LastRecordPayInfo.cState = 0;
            Log.d(TAG, "发射卡片交易成功信息");
            Publicfun.ShowCardInfo(g_CardBasicInfo, 3);//显示
            ...
            if (cResult == OK) {
                ...
                if (cResult == OK) {
                    ...
                    //发射卡片交易成功信息
                    Log.d(TAG, "发射卡片交易成功信息");
                    Publicfun.ShowCardInfo(g_CardBasicInfo, 4);//显示
    ...
    public static int Consume_OnlingCheckOut() {
        ...
        if (g_StationInfo.cCanStatusLimitFee == 0) {
            ...
            if (g_WorkInfo.lngPaymentMoney > lngSinglePayPswLim) {
                ...
                //转换密码
                cTradePWD[0] = (byte) ((g_CardBasicInfo.lngPaymentPsw / 100000) + 0x30);
                cTradePWD[1] = (byte) (((g_CardBasicInfo.lngPaymentPsw / 10000) % 10) + 0x30);
                cTradePWD[2] = (byte) (((g_CardBasicInfo.lngPaymentPsw / 1000) % 100 % 10) + 0x30);
                cTradePWD[3] = (byte) (((g_CardBasicInfo.lngPaymentPsw / 100) % 1000 % 100 % 10) + 0x30);
                cTradePWD[4] = (byte) (((g_CardBasicInfo.lngPaymentPsw / 10) % 10000 % 1000 % 100 % 10) + 0x30);
                cTradePWD[5] = (byte) ((g_CardBasicInfo.lngPaymentPsw % 100000 % 10000 % 1000 % 100 % 10) + 0x30);
...
//                }
        ...
        if (g_WorkInfo.cQrCodestatus == 2)//获取过订单号了
        {
            ...
            g_WorkInfo.cOtherQRFlag = 3;  //在线交易
    ...
    public static int Consume_WithholdCheckOut() {
        ...
        if (g_WorkInfo.cRunState == 0) {
            return NETCOM_ERR;
        }
        if (g_CommInfo.cWithholdInfoStatus != 0) {
            return NETCOM_ERR;
        }
...
//                }
    ...
    public static int WriteWorkRecord(byte Type) {
        ...
        if (Type == 0)        //消费 0x00
        {
            ...
            //交易金额(不含管理费)
            cMoneyTemp = g_CardBasicInfo.lngPayMoney - g_CardBasicInfo.lngManageMoney;
            Log.d(TAG, "交易金额(不含管理费):" + cMoneyTemp);
            ...
            //钱包卡余额
            cMoneyTemp = g_CardBasicInfo.lngWorkBurseMoney + g_CardBasicInfo.lngManageMoney;
    ...
    public static int WriteWorkRecord_Chase(int Type) {
        ...
        if (Type == 0)        //消费 0x00
        {
            ...
            //交易金额(不含管理费)
            cMoneyTemp = g_CardBasicInfo.lngWorkPayMoney;
            Log.d(TAG, "交易金额(不含管理费):" + cMoneyTemp);
            if (g_StationInfo.cPaymentUnit == 1) {
                cMoneyTemp = cMoneyTemp / 10;
            } else if (g_StationInfo.cPaymentUnit == 2) {
    ...
    public static int WriteChaseRecord(byte Type) {
        ...
        if (Type == 0)        //消费 0x00
        {
            ...
            //交易金额(不含管理费)
            cMoneyTemp = g_CardBasicInfo.lngChasePayMoney;
            if (g_StationInfo.cPaymentUnit == 1) {
                cMoneyTemp = cMoneyTemp / 10;
            } else if (g_StationInfo.cPaymentUnit == 2) {
                cMoneyTemp = cMoneyTemp / 100;
            ...
            //钱包卡余额
            cMoneyTemp = g_CardBasicInfo.lngChaseBurseMoney;
            stWasteBooks.cBurseMoney[0] = (byte) (cMoneyTemp & 0x000000FF);
            stWasteBooks.cBurseMoney[1] = (byte) ((cMoneyTemp & 0x0000FF00) >> 8);
            stWasteBooks.cBurseMoney[2] = (byte) ((cMoneyTemp & 0x00FF0000) >> 16);
    ...
    public static int WriteCardResetRecord(byte Type, int iBurseSID, long lngBurseMoney) {
        ...
        if (g_CardBasicInfo.iWorkBurseSID != iBurseSID) {
            cMoneyTemp = g_CardBasicInfo.lngPayMoney - lngBurseMoney;
        } else {
            cMoneyTemp = g_CardBasicInfo.lngWorkBurseMoney;
        }
        cMoneyTemp = g_CardBasicInfo.lngWorkBurseMoney;
    ...
    public static int WriteQrPayRecord(int cType) {
        ...
        //交易金额
        if ((g_WorkInfo.cOtherQRFlag == 1) || (g_WorkInfo.cOtherQRFlag == 4))
            cMoneyTemp = g_ThirdCodeResultInfo.lngPayMoney;
        else
            cMoneyTemp = g_OnlinePayInfo.lngPayMoney;

        Log.d(TAG, "交易金额:" + cMoneyTemp);
        stQrCodewasteBooks.cPaymentMoney[0] = (byte) (cMoneyTemp & 0x000000FF);
        stQrCodewasteBooks.cPaymentMoney[1] = (byte) ((cMoneyTemp & 0x0000FF00) >> 8);
        stQrCodewasteBooks.cPaymentMoney[2] = (byte) ((cMoneyTemp & 0x00FF0000) >> 16);
        //优惠金额
        if ((g_WorkInfo.cOtherQRFlag == 1) || (g_WorkInfo.cOtherQRFlag == 4))
            cMoneyTemp = g_ThirdCodeResultInfo.wPrivelegeMoney;
        else
            cMoneyTemp = g_OnlinePayInfo.lngPriMoney;

        stQrCodewasteBooks.cPrivelegeMoney[0] = (byte) (cMoneyTemp & 0x000000FF);
        stQrCodewasteBooks.cPrivelegeMoney[1] = (byte) ((cMoneyTemp & 0x0000FF00) >> 8);
        stQrCodewasteBooks.cPrivelegeMoney[2] = (byte) ((cMoneyTemp & 0x00FF0000) >> 16);
        //管理费
        if (g_WorkInfo.cOtherQRFlag == 1)
            cMoneyTemp = g_ThirdCodeResultInfo.wManageMoney;
        else
            cMoneyTemp = g_OnlinePayInfo.lngManageMoney;

        stQrCodewasteBooks.cManageMoney[0] = (byte) (cMoneyTemp & 0x000000FF);
        stQrCodewasteBooks.cManageMoney[1] = (byte) ((cMoneyTemp & 0x0000FF00) >> 8);
        stQrCodewasteBooks.cManageMoney[2] = (byte) ((cMoneyTemp & 0x00FF0000) >> 16);
        //钱包卡余额
        if ((g_WorkInfo.cOtherQRFlag == 1) || (g_WorkInfo.cOtherQRFlag == 4))
            cMoneyTemp = g_ThirdCodeResultInfo.lngBurseMoney;
        else
            cMoneyTemp = g_OnlinePayInfo.lngBurseMoney;

        stQrCodewasteBooks.cBurseMoney[0] = (byte) (cMoneyTemp & 0x000000FF);
        stQrCodewasteBooks.cBurseMoney[1] = (byte) ((cMoneyTemp & 0x0000FF00) >> 8);
        stQrCodewasteBooks.cBurseMoney[2] = (byte) ((cMoneyTemp & 0x00FF0000) >> 16);
...
//    //记录二维码交易流水 0:正元二维码主扫> 1：正元二维码被扫<   2：支付宝连接正元卡余额二维码被扫< 3：支付宝二维码被扫<
//    public static int WriteQrPayRecordA(int cType)
//    {
//        int  cResult;
//        long Index;
//        long Temp;
...
//        stQrCodewasteBooks.cShopUserID[1]= (byte) ((Temp&0xFF00)>>8);
//        //二维码单号
//        Log.d(TAG,"记录订单号:"+g_QrCodeResultInfo.lngOrderNum);
//        Temp=g_QrCodeResultInfo.lngOrderNum;
//        stQrCodewasteBooks.cQrCodeID[0]= (byte) (Temp&0x000000FF);
//        stQrCodewasteBooks.cQrCodeID[1]= (byte) ((Temp&0x0000FF00)>>8);
//        stQrCodewasteBooks.cQrCodeID[2]= (byte) ((Temp&0x00FF0000)>>16);
//        stQrCodewasteBooks.cQrCodeID[3]= (byte) ((Temp&0xFF000000)>>24);
//        //主卡帐号
//        Temp=g_QrCodeResultInfo.lngAccountID;
//        stQrCodewasteBooks.cAccountID[0]= (byte) (Temp&0x000000FF);
//        stQrCodewasteBooks.cAccountID[1]= (byte) ((Temp&0x0000FF00)>>8);
//        stQrCodewasteBooks.cAccountID[2]= (byte) ((Temp&0x00FF0000)>>16);
//        stQrCodewasteBooks.cAccountID[3]= (byte) ((Temp&0xFF000000)>>24);
//        //卡户姓名
//        Log.d(TAG,"记录姓名:"+g_QrCodeResultInfo.cAccName);
//        System.arraycopy(g_QrCodeResultInfo.cAccName,0,stQrCodewasteBooks.cAccName,0,g_QrCodeResultInfo.cAccName.length);
//        //交易时间
//        System.arraycopy(g_QrCodeResultInfo.cPaymentTime,0,stQrCodewasteBooks.cPaymentTime,0,4);
//
//        //终端流水号
//        lngPayRecordID=g_WasteQrBookInfo.WriterIndex+1;
//        Log.d(TAG,"记录流水号:"+lngPayRecordID);
//        stQrCodewasteBooks.cPayRecordID[0]= (byte) (lngPayRecordID&0x000000FF);
//        stQrCodewasteBooks.cPayRecordID[1]= (byte) ((lngPayRecordID&0x0000FF00)>>8);
//        stQrCodewasteBooks.cPayRecordID[2]= (byte) ((lngPayRecordID&0x00FF0000)>>16);
//        stQrCodewasteBooks.cPayRecordID[3]= (byte) ((lngPayRecordID&0xFF000000)>>24);
...
//        stQrCodewasteBooks.cPayType= (byte) cType;
//        //交易金额
//        Log.d(TAG,"记录交易金额:"+g_QrCodeResultInfo.lngPayMoney);
//        cMoneyTemp=g_QrCodeResultInfo.lngPayMoney;
//        stQrCodewasteBooks.cPaymentMoney[0]= (byte) (cMoneyTemp&0x000000FF);
//        stQrCodewasteBooks.cPaymentMoney[1]= (byte) ((cMoneyTemp&0x0000FF00)>>8);
//        //优惠金额
//        cMoneyTemp=g_QrCodeResultInfo.wPrivelegeMoney;
//        stQrCodewasteBooks.cPrivelegeMoney[0]= (byte) (cMoneyTemp&0x000000FF);
//        stQrCodewasteBooks.cPrivelegeMoney[1]= (byte) ((cMoneyTemp&0x0000FF00)>>8);
//        //管理费
//        cMoneyTemp=g_QrCodeResultInfo.wManageMoney;
//        stQrCodewasteBooks.cManageMoney[0]= (byte) (cMoneyTemp&0x000000FF);
//        stQrCodewasteBooks.cManageMoney[1]= (byte) ((cMoneyTemp&0x0000FF00)>>8);
//        //钱包卡余额
//        cMoneyTemp=g_QrCodeResultInfo.lngBurseMoney;
//        stQrCodewasteBooks.cBurseMoney[0]= (byte) (cMoneyTemp&0x000000FF);
//        stQrCodewasteBooks.cBurseMoney[1]= (byte) ((cMoneyTemp&0x0000FF00)>>8);
//        stQrCodewasteBooks.cBurseMoney[2]= (byte) ((cMoneyTemp&0x00FF0000)>>16);
//
...
//    }
    ...
    public static int WriteFacePayRecord(int cType) {
        ...
        cMoneyTemp = g_OnlinePayInfo.lngPayMoney;
        Log.d(TAG, "交易金额:" + cMoneyTemp);
        stFaceWasteBooks.cPaymentMoney[0] = (byte) (cMoneyTemp & 0x000000FF);
        stFaceWasteBooks.cPaymentMoney[1] = (byte) ((cMoneyTemp & 0x0000FF00) >> 8);
        stFaceWasteBooks.cPaymentMoney[2] = (byte) ((cMoneyTemp & 0x00FF0000) >> 16);
        //优惠金额
        cMoneyTemp = g_OnlinePayInfo.lngPriMoney;
        stFaceWasteBooks.cPrivelegeMoney[0] = (byte) (cMoneyTemp & 0x000000FF);
        stFaceWasteBooks.cPrivelegeMoney[1] = (byte) ((cMoneyTemp & 0x0000FF00) >> 8);
        stFaceWasteBooks.cPrivelegeMoney[2] = (byte) ((cMoneyTemp & 0x00FF0000) >> 16);
        //管理费
        cMoneyTemp = g_OnlinePayInfo.lngManageMoney;
        stFaceWasteBooks.cManageMoney[0] = (byte) (cMoneyTemp & 0x000000FF);
        stFaceWasteBooks.cManageMoney[1] = (byte) ((cMoneyTemp & 0x0000FF00) >> 8);
        stFaceWasteBooks.cManageMoney[2] = (byte) ((cMoneyTemp & 0x00FF0000) >> 16);
        //钱包卡余额
        cMoneyTemp = g_OnlinePayInfo.lngBurseMoney;
        stFaceWasteBooks.cBurseMoney[0] = (byte) (cMoneyTemp & 0x000000FF);
        stFaceWasteBooks.cBurseMoney[1] = (byte) ((cMoneyTemp & 0x0000FF00) >> 8);
        stFaceWasteBooks.cBurseMoney[2] = (byte) ((cMoneyTemp & 0x00FF0000) >> 16);
    ...
    //日累统计、餐累统计、写末笔营业时段号(TYPE:0 消费 1 冲正）
