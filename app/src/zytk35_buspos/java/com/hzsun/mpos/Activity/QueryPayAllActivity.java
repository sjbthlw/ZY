 ...
 * 查询总界面
 */
public class QueryPayAllActivity extends
    ...
    private void initListView() {
        ...
        stringList.add("时段查询");
        stringList.add("时段营业统计");
        stringList.add("当日营业统计");
        ...
        stringList.add("平台流水明细");

        menuListviewAdapter = new MenuListviewAdapter(this, stringList);
        lv_Query.setAdapter(menuListviewAdapter);
        lv_Query.setOnItemClickListener(this);
    }

    //0.车价时段查询
    private void QueryVehiclePriceTime(int iPosition) {
        ...
        String strTemp = "";
        String strTitle = "时段查询";

        //营业分组时间
        strTemp = String.format("当前营业时间:%02d:%02d - %02d:%02d", g_WorkInfo.cStartTime[0], g_WorkInfo.cStartTime[1]
                , g_WorkInfo.cEndTime[0], g_WorkInfo.cEndTime[1]);
        OptionItemList.add(strTemp);

        //当前营业车价
        strTemp = String.format("当前营业交易金额:%d.%02d", g_WorkInfo.wBookMoney / 100, g_WorkInfo.wBookMoney % 100);
        OptionItemList.add(strTemp);
        ...
        //营业分组时间
        for (int i = 0; i < pBuinessInfo.size(); i++) {
            strTemp = String.format("ID:%d - 时段：%02d:%02d-%02d:%02d   交易金额:%d.%02d",
                    pBuinessInfo.get(i).cBusinessID,
                    pBuinessInfo.get(i).cStartTime[0], pBuinessInfo.get(i).cStartTime[1],
                    pBuinessInfo.get(i).cEndTime[0], pBuinessInfo.get(i).cEndTime[1],
                    g_OddKeyInfo.wKeyMoney[i] / 100, g_OddKeyInfo.wKeyMoney[i] % 100);
...
//        String strAccID="";
//        try {
//            long lngAccountID=Long.parseLong(strAccID);
//            Log.d(TAG,"数据转换:"+lngAccountID);
//        } catch (Exception e) {
//            e.printStackTrace();
//            Log.d(TAG,"数据转换异常:"+e.getMessage());
//        }
        ...
        if (g_StationInfo.cCanPermitStat == 1) {
            ...
            if (cResult == 0) {
                //时段营业笔数 - 时段营业总额
                strTemp = String.format("时段营业笔数：%d", g_RecordInfo.wTotalBusinessSum);
                OptionItemList.add(strTemp);

                //时段营业总额
                strTemp = String.format("时段营业总额：%d.%02d", g_RecordInfo.lngTotalBusinessMoney / 100, g_RecordInfo.lngTotalBusinessMoney % 100);
                OptionItemList.add(strTemp);

                //末笔交易营业号
                strTemp = String.format("末笔交易营业号：%d", g_RecordInfo.cLastBusinessID);
                OptionItemList.add(strTemp);
                ...
                //时段营业笔数 - 时段营业总额
                strTemp = String.format("时段营业笔数：%d", 0);
                OptionItemList.add(strTemp);

                //时段营业总额
                strTemp = String.format("时段营业总额：%d.%02d", 0, 0);
                OptionItemList.add(strTemp);
            ...
            //营业分组时间
            for (int i = 0; i < pBuinessInfo.size(); i++) {
                strTemp = String.format("ID:%d - 时段：%02d:%02d-%02d:%02d", pBuinessInfo.get(i).cBusinessID,
                        pBuinessInfo.get(i).cStartTime[0], pBuinessInfo.get(i).cStartTime[1],
                        pBuinessInfo.get(i).cEndTime[0], pBuinessInfo.get(i).cEndTime[1]);
    ...
    private void QueryLastRecord(int iPosition) {
        ...
        if (OptionItemList == null) {
            ...
            startActivityForResult(new Intent(this, QueryRecordActivity.class)
    ...
    private ArrayList<String> GetToRecordList() {
        ...
        for (i = 0; i < iRecordCount; i++) {
            ...
            if (LastPayDate >= QRLastPayDate) //卡交易记录流水
            {
                ...
                //姓名
                try {
                    strName = new String(pWasteBooks.cAccName, "GB2312");
    ...
    private void QueryListRecordInfo(int iPosition) {
        ...
        //交易流水记录
        strTemp = String.format("交易记录数量：%d/%d", g_WasteBookInfo.WriterIndex % MAXBOOKSCOUNT, MAXBOOKSCOUNT);
        OptionItemList.add(strTemp);
    ...
    private void QueryDeviceSystemPare(int iPosition) {
        ...
        //本机IP
        if (g_WorkInfo.cNetDevStatus == 0) {

            if (g_LocalNetStrInfo.IPMode == 0) //DHCP
            {
                String strIPaddr = "";
                strIPaddr = Publicfun.getIP(this);
                OptionItemList.add("本机IP(DHCP)：" + strIPaddr);
            } else {
                OptionItemList.add("本机IP(Static)：" + g_LocalNetStrInfo.strLocalIP);
                //OptionItemList.add("本机IP(Static)："+strIPaddr);
            }
        } else if (g_WorkInfo.cNetDevStatus == 1) {
        } else if (g_WorkInfo.cNetDevStatus == 2) {
        }
    ...
    private ArrayList<String> GetFaceRecordList() {
        ...
        for (i = 0; i < iRecordCount; i++) {
            ...
            //姓名
            try {
                strName = new String(pWasteFacePayBooks.cAccName, "GB2312");
            } catch (UnsupportedEncodingException e) {
                e.printStackTrace();
            }
            //交易类型
    ...
    private void QueryFaceDiscernInfo(int iPosition) {
        ...
        //人脸瞳距
        strTemp = String.format("瞳距：%d", g_FaceIdentInfo.iPupilDistance);
        OptionItemList.add(strTemp);
        ...
        //平台版本号
        strTemp = String.format("平台版本号：%d", g_FaceCodeInfo.lngPlatVer);
        OptionItemList.add(strTemp);
    ...
    private void QueryBillsDetail() {
        startActivity(new Intent(this, QueryPlatRecActivity.class));
    }
    ...
    public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
        switch (position) {
            case 0: //车价时段查询
                QueryVehiclePriceTime(position);
                break;
            case 1: //餐累统计
                QueryTimeAmountTotal(position);
                break;
            case 2:  //日累统计
                QueryDayAmountTotal(position);
                break;
            case 3:   //最近流水查询
                QueryLastRecord(position);
                break;
            case 4: //未传流水信息
                QueryNoUpRecord(position);
                break;
            case 5:  //名单记录信息
                QueryListRecordInfo(position);
                break;
            case 6:  //本机系统信息
                QueryDeviceSystemPare(position);
                break;
            case 7: //人脸流水查询
                QueryFaceLastRecord(position);
                break;
            case 8:  //人脸识别信息
                QueryFaceDiscernInfo(position);
                break;
            case 9: //平台流水明细
                QueryBillsDetail();
                break;
