...
import static com.hzsun.mpos.Global.Global.LAN_OLDDEVTYPE;
...
import static com.hzsun.mpos.Global.Global.g_NetEventMsg;
...
//            g_WorkInfo.lngOSTime=g_WorkInfo.lngOffLineTime+NODATARECVTIME*10;
//        }
//        else
//        {
...
//            g_WorkInfo.lngOffLineTime=g_WorkInfo.lngOSTime;
//            g_WorkInfo.lngOnLineTime=g_WorkInfo.lngOSTime;
//            g_WorkInfo.lngOSTime=g_WorkInfo.lngOnLineTime+RECONNECTCOUNT*20;
//        }
        g_WorkInfo.cRunState = 5; //重新连接统一调度服务
        g_WorkInfo.lngOffLineTime = g_WorkInfo.lngOSTime;
        g_WorkInfo.lngOnLineTime = g_WorkInfo.lngOSTime;
        ...
        ControlBusiness();
        //TCP连接服务器
        g_NetEventMsg.type = EVT_NULL;
        g_NetEventMsg.cConnectState = 0;
        g_NetEventMsg.cMsgResState = 0;
        g_NetEventMsg.lngNetTimeout = 0;
        g_NetEventMsg.wMsgTimeoutCount = 0;
        isOnline = false;
    ...
    private void ConnectServer(String strServerIP, int iServerPort, byte mode) {
        ...
        try {
            ...
            if (socket == null || socket.isClosed()) {
                socket = null;
                g_NetEventMsg.type = EVT_CONNECT;
                    ...
                    public void handleMessage(Message msg) {
                        switch (msg.what) {
                            ...
                            case EVT_CONNECT:
                                ...
                                DeviceJoin_Send();
                                g_NetEventMsg.type = EVT_DEVICEJOIN;

                                // 步骤4:通知主线程,将接收的消息显示到界面
                                if (g_UIMainHandler != null)
                                    g_UIMainHandler.sendEmptyMessage(CONNECTOK);
                            ...
                            case EVT_SIGNIN:
                                ...
                                if (cResult == OK) {
                                    ...
                                    Log.i(TAG, "设备签到成功");
                                    g_NetEventMsg.type = EVT_ONGOING;
                                } else {
                                    g_WorkInfo.cNetworkErrFlag = 1;
                                    if (cResult == 4) {
                                        //平台客户号不一致 界面提示
...
//                    break;
    ...
    public void DeviceJoin_Send() {
        ...
        */
        ...
        //0x1C	YT21X 多媒体POS机 //设备型号：多媒体人脸POS机 YT223是0x24，324是0x25
        if(g_LocalInfo.iDeviceType==0)
            s_cSendDataBuf[n++] = LAN_DEVTYPE;
        else
            s_cSendDataBuf[n++] = LAN_OLDDEVTYPE;
        //终端序列号
...
//        上传照片文件夹	16字节	字符形式，抓拍照片以站点号+站点流水号+账号命名，如“6_1_12345678.png”
    ...
    public int DownParaOver() {
        ...
        if (LAN_EP_MONEYPOS == g_StationInfo.iStationClass) {
            ...
            if (g_UIMainHandler != null)
                g_UIMainHandler.sendEmptyMessage(DOWNPARAOVER);
        } else {
            //联机运行
    ...
    public void NetTimeOverProcess() {
        int cResult;
        //连接服务器超时
        if (g_NetEventMsg.type != EVT_NULL) {
            //连接服务器
            if (((g_NetEventMsg.type == EVT_CONNECT)) && (g_NetEventMsg.cConnectState == 0)) {
                g_NetEventMsg.lngNetTimeout++;
                if (g_NetEventMsg.lngNetTimeout >= COMTIMEOUT * 10) {
                    g_NetEventMsg.cConnectState = EVT_NETTIMEOUT;
                    g_NetEventMsg.lngNetTimeout = 0;
            ...
            //接收服务器报文(签到过程)
            if ((g_NetEventMsg.type == EVT_DEVICEJOIN)
                    || (g_NetEventMsg.type == EVT_SIGNIN)) {
                if ((g_NetEventMsg.cMsgResState == 0) && (g_NetEventMsg.cConnectState == 1)) {
                    g_NetEventMsg.lngNetTimeout++;
                    if (g_NetEventMsg.lngNetTimeout >= MSGTIMEOUT * 10) {
                        g_NetEventMsg.cMsgResState = EVT_NETTIMEOUT;
                        g_NetEventMsg.lngNetTimeout = 0;
            ...
            //接收服务器报文(运行过程)
            if ((g_CommInfo.cRecvWaitState == 1) && (g_NetEventMsg.cMsgResState == 0)) {
                g_NetEventMsg.lngNetTimeout++;
                //判断卡户校验超时
                if ((g_CommInfo.lngSendComStatus & 0x00000010) == 0x00000010) {
                    if (g_NetEventMsg.lngNetTimeout >= 1 * 15) {
                        Log.i(TAG, "卡户校验报文超时");
                        Message message = Message.obtain();
                        ...
                        g_CommInfo.cRecvWaitState = 0;
                        g_CommInfo.lngSendComStatus = 0;
                        g_NetEventMsg.wMsgTimeoutCount++;//记录超时次数，达到超时次数上限，重新连接服务器
                    }
                }
                //正元二维码有效性校验
                else if ((g_CommInfo.lngSendComStatus & 0x00000800) == 0x00000800) {
                    if ((g_NetEventMsg.lngNetTimeout >= 10 * 3)//超过2秒
                            || ((g_WorkInfo.cOtherQRFlag == 3) && (g_NetEventMsg.lngNetTimeout >= 25)))//在线卡片交易
                    {
                ...
                //第三方二维码有效性校验
                    ...
                    if (g_NetEventMsg.lngNetTimeout >= 10 * 30)//超过20秒
                    {
                ...
                //在线冲正超时
                    ...
                    if (g_NetEventMsg.lngNetTimeout >= 10 * 5)//超过10秒
                    {
                        Log.e(TAG, "在线冲正超时");
                        Message message = Message.obtain();
                ...
                //第三方代扣
                    ...
                    if (g_NetEventMsg.lngNetTimeout >= 10 * 10)//超过10秒
                    {
                        Log.d(TAG, "第三方代扣超时");
                        Message message = Message.obtain();
                    ...
                    if (g_NetEventMsg.lngNetTimeout >= 10 * 6)//超过3秒
                    {
                        Log.i(TAG, "获取ftp失败");
                        g_NetEventMsg.cMsgResState = EVT_NETTIMEOUT;
                        g_NetEventMsg.lngNetTimeout = 0;
                ...
                //第三方末笔订单号查询
                    ...
                    if (g_NetEventMsg.lngNetTimeout >= 10 * 10)//超过3秒
                    {
                        ...
                        g_NetEventMsg.wMsgTimeoutCount++;
                    }
                } else {
                    if (g_NetEventMsg.lngNetTimeout >= MSGTIMEOUT * 10) {
                ...
                if (g_NetEventMsg.wMsgTimeoutCount > MSGTIMEOUTCOUNT) {
                    Log.i(TAG, "==========(运行过程)发送报文超时次数超出上限============");
                    //memset((void*)(&g_NetEventMsg), 0, sizeof(NETEVENTMSG));
                    g_WorkInfo.cConnState = 0;
                    g_WorkInfo.cUpdateState = 0;
                    g_WorkInfo.cRunState = 2;
                    SendHandler.sendEmptyMessage(EVT_DISCONNECT);
