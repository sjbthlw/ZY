...
//        }
    ...
    public void run() {
        ...
        while (isStart) {
            try {
                ...
                for (UsbFile otgFile : UFile.listFiles()) {
                    ...
                    if (!fileName.endsWith(".v10")) {
                        ...
                        if (data != null) {
                            ...
                            Log.d(TAG, "人脸照片文件大小:" + iFileLen);
                            if(getImageType(data) == 0) {    //判断图片格式 png jpg bmp
                                if((iFileLen < (512 * 1024))&&(iFileLen > (20 * 1024))){
                                    Bitmap bitmap = BitmapFactory.decodeByteArray(data, 0, data.length);
                                    featureSingle = bitmap2feature(bitmap, fileName);
                                    if (featureSingle == null) {
                                        continue;
                                    }
                                }else {
                                    FileUtils.writeToFile(Publicfun.toData(System.currentTimeMillis()) +" "+
                                            fileName + ",人脸照片文件大小超出范围,拷贝图片失败" + "\r\n", ZYTKFacePath, "USBFeatureCodeExt.txt");
                                    continue;
                                }
                            }else {
                                Log.e(TAG, "U盘的图片格式不正确");
                                FileUtils.writeToFile(strTemp + ",U盘的图片格式不正确" + "\r\n", ZYTKFacePath, "featureCodeExt.txt");
                                continue;
                            }
                        } else {
                            FileUtils.writeToFile(Publicfun.toData(System.currentTimeMillis()) +" "+
                ...
                usbManager.sd2otg("FeaturePath/", new File(ZYTKFacePath + "/USBFeatureCodeExt.txt"));
                break;
            } catch (IOException e) {
                e.printStackTrace();
                isStart = false;
        ...
        Log.d(TAG, "加载成功，特征码数量:" + g_FaceIdentInfo.iListNum);
        Message msg = Message.obtain();
        msg.what = 2;
        if (handler != null)
            handler.sendMessage(msg);
    ...
    private byte[] bitmap2feature(Bitmap mBitmap, String fileName) {
        if (mBitmap == null) {
            FileUtils.writeToFile(Publicfun.toData(System.currentTimeMillis()) +" "+
                    fileName + ",拷贝图片失败" + "\r\n", ZYTKFacePath, "USBFeatureCodeExt.txt");
            return null;
        }
        byte[] mFaceFeatures = new byte[FFSize];
        THFI_FacePos[] facePos = new THFI_FacePos[1];
        facePos[0] = new THFI_FacePos();
        byte[] pixelsBGR = FaceFunction.getPixelsBGR(mBitmap);
        if (pixelsBGR == null) {
            Log.e(TAG, fileName + "下载图片的像素太大");
            FileUtils.writeToFile(Publicfun.toData(System.currentTimeMillis()) +" "+
                    fileName + ",下载图片的像素太大" + "\r\n", ZYTKFacePath, "USBFeatureCodeExt.txt");
            return null;
        }
        ...
        if (faceNum == 1) {
            mFaceFeatures = FaceFunction.faceFeatures(pixelsBGR, mBitmap.getWidth(), mBitmap.getHeight(), 30, facePos);
            if (mFaceFeatures == null) {
                Log.e(TAG, "图片转换特征码失败");
                FileUtils.writeToFile(Publicfun.toData(System.currentTimeMillis()) +" "+
                        fileName + ",图片转换特征码数据失败" + "\r\n", ZYTKFacePath, "USBFeatureCodeExt.txt");
                return null;
            }
        } else if (faceNum > 1) {
            Log.e(TAG, fileName + "图片多人脸");
            FileUtils.writeToFile(Publicfun.toData(System.currentTimeMillis()) +" "+
                    fileName + ",图片多人脸" + "\r\n", ZYTKFacePath, "USBFeatureCodeExt.txt");
            return null;
        } else if (faceNum < 1) {
            Log.e(TAG, fileName + "图片无人脸");
            FileUtils.writeToFile(Publicfun.toData(System.currentTimeMillis()) +" "+
                    fileName + ",图片无人脸" + "\r\n", ZYTKFacePath, "USBFeatureCodeExt.txt");
            return null;
        }
