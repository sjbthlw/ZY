...
import static com.hzsun.mpos.Activity.StartActivity.SET_SHELL_MSG;
...
import static com.hzsun.mpos.Public.FileUtils.getCurrentFolderSize;
import static com.hzsun.mpos.Public.FileUtils.orderByDate;
...
public class Publicfun {
    ...
    public static void InitWorkPara() {
        ...
        g_HttpCommInfo = new HttpCommInfo();
    ...
    public static int InitSysParam() {
        ...
        CheckAndDelFile(FACEPICCOUNT,0, PicPath);
        Log.e(TAG, "=========删除PicPath耗时:" + (System.currentTimeMillis() - lngStart));
        lngStart=System.currentTimeMillis();
        CheckAndDelFile(0, PHOTOFILESIZE, PhotoPath);
        Log.e(TAG, "=========删除PhotoPath耗时:" + (System.currentTimeMillis() - lngStart));
    ...
    public static int FactoryReset() {
        ...
        //RunShellCmd("rm -rf " + ZYTKFacePath);
        //RunShellCmd("rm -rf " + ZYTK35Path);
        //RunShellCmd("rm -rf " + ZYTKPath + "DeviceNetPara.ini");
        //RunShellCmd("rm -rf " + ZYTKPath + "DeviceProType.ini");
        RunShellCmd("reboot");
        return 0;
    }
    ...
    public static void SetLocalAllPara() {
        ...
        g_LocalInfo.cInputMode = 1;                    //输入方式
        //定义的定额值
        for (i = 0; i < 6; i++) {
            g_LocalInfo.wBookMoney[i] = (500 + i * 100);
        }
        g_LocalInfo.cBookSureMode = 1;                //定额方式是否需要确认(默认需要 1)
        ...
        g_LocalInfo.cPowerSaveTimeA = 5;             //启用省电模式时长 5-30分钟
...
//        System.arraycopy(cLocalMac,0,g_LocalNetInfo.cMAC,0,6);
    ...
    public static void SetLocalNetStrCfg(LocalNetStrInfo pLocalNetStrInfo) {
        ...
        if (pLocalNetStrInfo.IPMode == 1)
            NetInfoMap.put("IPMode", "static");
        else
            NetInfoMap.put("IPMode", "dhcp");
...
//        Log.d(TAG,"计算毫秒："+lngTimeMillis);
    ...
    public static int SetDateTime(byte[] cCurDateTime) {
        ...
        if (g_UIStartHandler != null) {
            ...
            Log.e(TAG, "g_UIStartHandler 不存在");
        }
    ...
    //发送shell指令到引导程序运行
    public static void SendShellCmd(String strShellCmd) {

        if (g_UIStartHandler != null) {
            Log.e(TAG, "--------SendShellCmd:" + strShellCmd);
            Message msg = Message.obtain();
            msg.obj = strShellCmd;
            msg.what = SET_SHELL_MSG;
            g_UIStartHandler.sendMessage(msg);
        }
        return ;
    }
    ...
    public static int Led_Init(char cMode) {
        if (cMode == 0) {
            ...
            g_Nlib.LedShow((char) 2, 1);
            g_Nlib.LedShow((char) 3, 1);
        }
        return 0;
    }
    ...
    public static void ShowCardPaying() {
        ...
        if (g_UICardHandler != null)
            g_UICardHandler.sendMessage(message);
    }
    ...
    public static void ShowCardErrorInfo_Dispel(int iErrorCode) {
        ...
        if (iErrorCode > 0) {
            ...
            message.obj = Strlist;
            message.what = 100;
            if (g_UIRDCardHandler != null)
                g_UIRDCardHandler.sendMessage(message);
        }
    }
    ...
    //摄像头操作
    public static void CameraDeal( int cType) {

        Message message = Message.obtain();
        message.obj = cType;
        message.what = CAMERADEAL;

        Log.d(TAG,"操作摄像头状态:"+cType);
        if (g_UICardHandler != null)
            g_UICardHandler.sendMessage(message);
    }
    ...
    public static void PowerSaveDeal( int cType) {
        ...
        if (g_UIMainHandler != null)
            g_UIMainHandler.sendMessage(message);
            ...
            if (g_BuinessInfo.get(i).cBusinessID != 0) {
                ...
                if ((iNowTime >= iStartTime) && (iNowTime <= iEndTime)) {
                    ...
                    if (iTimeID > 5)
                        iTimeID = 5;
                    //Log.i(TAG,"餐次改变定额值");
                    g_WorkInfo.wBookMoney = g_LocalInfo.wBookMoney[iTimeID];
...
//            if(split[0].equals("99999"))
    ...
    public static int CalcRatio(int iMole, int iDeno) {
        java.text.NumberFormat nt = NumberFormat.getPercentInstance();
        //设置百分数精确度2即保留两位小数
...
//        星期日：Sunday 缩写：Sun.
            ...
            if (iMode == 0)    //shell脚本执行
            {
                ...
                if (strTmpList.length > 1) {
                    ...
                    if (strTmpVer.length() > 23) {
                        strVer = strTmpVer.substring(12, 24);
                    }
                }
                if (strVer.equals(""))
                    return 1;
    ...
    public static void CheckAndDelFile(int iLogCnt, String strPath) {
        ...
        //删除原有的log
        RunShellCmd("rm -f " + LogPathOld );
    }
    //判断文件夹大小，并删除对应最老的文件
    public static void CheckAndDelFile(int iFileCnt,long lngFileLength, String strPath) {

        if (!TextUtils.isEmpty(strPath)){
            File[] files = new File(strPath).listFiles();
            if (files != null) {
                if ((lngFileLength != 0)&&(getCurrentFolderSize(files) > lngFileLength)) {

                    List<String> fileNamesOut = orderByDate(files);//按文件的最近修改时间排序
                    for (int i = 0; i < (fileNamesOut.size() / 20) ; i++) {
                        File file = new File(strPath + fileNamesOut.get(i));
                        if (file.exists() && file.isFile()) {
                            file.delete();
                        }
                ...
                if ((iFileCnt != 0)&&(files.length > iFileCnt)) {

                    List<String> fileNamesOut = orderByDate(files);//按文件的最近修改时间排序
                    for (int i = 0; i < (files.length - iFileCnt) ; i++) {
                        File file = new File(strPath + fileNamesOut.get(i));
                        if (file.exists() && file.isFile()) {
                            file.delete();
                        }
...
//                            0
    ...
    //获取文件夹文件数
    public static int GetSDFileCnt(final File f, List<String> itemsExport) {
        int iFileCount = 0;
        UsbFile usbFile = null;
        if (f.isDirectory()) { //如果选择是个文件夹
            for (File sdFile : f.listFiles()) {
                if ((sdFile.isDirectory()) && (itemsExport.contains(sdFile.getName())))
                    iFileCount +=  GetSDFileCnt(sdFile);
                else
                    iFileCount++;
            }
...
//    //控制7寸屏幕开关
//    public static  void LCD7Control(int iState)
//    {
//        if (iState == 1) {
...
//    }
    ...
    //控制7寸屏幕背光开关
    public static  void LCD7BacklightControl(int iState)
    {
        if (iState == 1) {
            RunShellCmd("echo 1 > /sys/class/gpio/gpio92/value");
        } else {
            RunShellCmd("echo 0 > /sys/class/gpio/gpio92/value");
        }
    }
