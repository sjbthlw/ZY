...
import static com.hzsun.mpos.Activity.CardActivity.CARD_SHOWTOTAL;
...
import static com.hzsun.mpos.Pos.MoneyPoint.MoneyPoint_CheckOut;
...
public class Keyoperpro {
    ...
    public static void CheckOut() {
        ...
        if ((g_WorkInfo.cCheckOutState == 0)
            ...
            if ((g_CardInfo.cAuthenState == 2) && (g_CardInfo.cExistState == 1)
                    && (g_WorkInfo.cInfoStepState == 0) && (g_WorkInfo.cStartWCardFlag == 0)) {
                ...
                if ((g_WorkInfo.cStartWCardFlag != 0)||(g_WorkInfo.cInfoStepState != 0))
                    return;
                //判断设备类型
                ...
                if (g_StationInfo.iStationClass == LAN_EP_CONSUMEPOS) {
                    ...
                    cResult = MoneyPoint_CheckOut();
                }

                if (cResult == OK) {
                    if (g_LocalInfo.cPlayVoiceMoneyFlag == 1) {
                        soundPlayThread ThreadSoundPlay = new soundPlayThread(g_CardBasicInfo.lngPayMoney);
                        ThreadSoundPlay.start();
                    } else {
                        VoicePlay("zhifuchenggong");
                    }
                    Publicfun.ShowCardInfo(g_CardBasicInfo, CARD_SHOWTOTAL); //发射卡片交易成功信息
                } else {
                    Publicfun.ShowCardErrorInfo(cResult);
                    //判断存储失败和时钟故障
