...
//        }
    ...
    public static int DoPrint(WasteBooks stWasteBooks) {
        ...
        //项目名称
        cShowTemp = String.format("      智慧一卡通系统\n");
        try {
        ...
        String cNameTemp = "";
        try {
            cNameTemp = ByteToString(stWasteBooks.cAccName, "GB2312");
...
//        }
    ...
    //打印内容(二维码)
    public static int DoPrintQR(WasteQrCodeBooks stWasteBooks){
        int i;
        String cShowTemp = "";
        byte[] szBuf = new byte[512];
        byte[] buf = new byte[512];
        ...
        //1凭证类型
        cShowTemp = String.format("         支付消费凭证\r\r");
        try {
            szBuf = cShowTemp.getBytes("gbk");
        } catch (UnsupportedEncodingException e) {
            e.printStackTrace();
        ...
        lngTemp = (stWasteBooks.cPaymentMoney[0] & 0xff) + (stWasteBooks.cPaymentMoney[1] & 0xff) * 256;
        //交易类型
        cShowTemp = String.format("支付金额*:%d.%02d元\r", lngTemp / 100, lngTemp % 100);
        try {
            szBuf = cShowTemp.getBytes("gbk");
        } catch (UnsupportedEncodingException e) {
            e.printStackTrace();
...
//        }
    ...
    //接收打印
    public static int PrintReceiptQR(WasteQrCodeBooks stQRWasteBooks) {
        int lResult;
        ...
        //执行打印
        lResult = DoPrintQR(stQRWasteBooks);
        if (lResult != 0) {
            Log.d(TAG, "打印失败");
            return 3;
        } else {
