...
//    public static final int RDCARD_RWEND = 9;     //卡片重读卡结束
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (g_ExtDisplay != null) {
            try {
                cardPreActivity = new CardPreActivity(RecordDispelActivity.this, this, g_ExtDisplay);
                cardPreActivity.ShowDisplay();
            } catch (Exception e) {
                e.printStackTrace();
                Log.e(TAG, "冲正显示副屏异常:" + e.getMessage());
            } finally {
        ...
        slngTimer = 0;
        slngOSTimer = System.currentTimeMillis();
        sDispelCardOK = 0;
        ShowDateTime();
        ShowShoperName();
        ShowTotalMoneySun();
    ...
    private void TimeScanWork() {
        ...
        //判断超时退出
        if (((System.currentTimeMillis() - slngOSTimer) > 55 * 1000) && (g_CardInfo.cExistState == 0)) {
...
//        }
        ...
        cardPreActivity.ShowShoperName(strShowTemp);
    }
    ...
    private void ShowCardInfo_Dispel(CardBasicParaInfo pCardBasicInfo, byte ucType) {
        ...
        if (sTypeTemp == RDCARD_INMONEY) {
            //卡片信息
            tv_Name.setText("");
            tv_Bursemoney.setText("0.00");
            tv_Paymoney.setText("0.00");
            tv_Prompt.setText("请冲正");
        } else if (sTypeTemp == RDCARD_READOK) {
            //卡片信息
        ...
        } else if (sTypeTemp == RDCARD_PAYOK) //显示卡片交易冲正成功
        {
            //卡片信息
            byte[] accName = GetUserName(g_CardBasicTmpInfo.cAccName);
            try {
        ...
        } else if (sTypeTemp == RDCARD_PAYOKONLING)   //显示在线交易冲正成功
        {
            //卡片信息
            byte[] accName = GetUserName(g_CardBasicTmpInfo.cAccName);
            try {
            ...
            g_CardBasicTmpInfo.lngWorkBurseMoney+=g_CardBasicTmpInfo.lngPayMoney;//钱包余额=原有金额+冲正金额
            if (g_CardBasicTmpInfo.lngWorkBurseMoney >= 0) {
                cShowTemp = String.format("%d.%02d", g_CardBasicTmpInfo.lngWorkBurseMoney / 100, g_CardBasicTmpInfo.lngWorkBurseMoney % 100);
                tv_Bursemoney.setText(cShowTemp);
            } else {
        ...
        } else if (sTypeTemp == RDCARD_ONLING)   //显示在线交易冲正信息
        {
            byte[] accName = GetUserName(g_CardBasicTmpInfo.cAccName);
            try {
                cShowTemp = ByteToString(accName, "GB2312");
    ...
    public void ShowOnLinePayInfo(LastRecordPayInfo pLastRecordPayInfo, int cType) {
        ...
        Log.i(TAG, "显示冲正信息");
        ShowCardInfo_Dispel(pCardBasicInfo, (byte) RDCARD_ONLING);
        cardPreActivity.ShowCardInfo_Dispel(pCardBasicInfo, (byte) RDCARD_ONLING);
    }
        ...
        switch (event.getKeyCode()) {
            ...
            case KeyEvent.KEYCODE_ENTER:
                ...
                if ((g_WorkInfo.IsRecordDispelOver == 0) && (g_WorkInfo.IsDispelRecord == 1)) {
                    ...
                    if (cResult == OK) {
                        VoicePlay("recorddispel_ok");
                        Publicfun.ShowCardInfo_Dispel(g_CardBasicInfo, RDCARD_PAYOK);//显示
                    } else {
    ...
    protected void onDestroy() {
        ...
        if (g_UIRDCardHandler != null) {
            g_UIRDCardHandler.removeCallbacksAndMessages(null);
            g_UIRDCardHandler = null;
        }
