...
import static com.hzsun.mpos.Global.Global.g_UIRDCardHandler;
...
public class SerialWorkTask {
    ...
    public static int s_TradeState;
    ...
    public static int s_TradeCode = -1;//-1代表初始状态
                ...
                if (len > 0) {
                    ...
                    switch (cmd) {
                        ...
                        case 0xC3:
                            ...
                            if (s_TradeState == 3) {
                                if (g_WorkInfo.cOtherQRFlag == -1) {
                                    ...
                                    if ((g_WorkInfo.cOtherQRFlag == 0
                                            ...
                                            && (s_TradeCode == 0x02 || s_TradeCode == 0x05 || s_TradeCode == 0x06 || s_TradeCode == 0x07 || s_TradeCode == 0x08 || s_TradeCode == 0x15)) {
                                        //可以取消支付
                                        SerialFunc.sendCancelPayReplyPacket();
                                        mHandler.removeMessages(FINISH_QR_SCAN);
                                        mHandler.sendEmptyMessageDelayed(FINISH_QR_SCAN, TIMEOUT);
                            ...
                            } else if (s_TradeState == 4 || s_TradeState == 0) {
                                if (s_TradeCode == 0x05
                                        || s_TradeCode == 0x99
                                        || s_TradeCode == 0x91
                                        || (s_TradeCode == -1 && !lastPayResult.isSuccess())
                                        || (s_TradeState == 0 && s_TradeCode == 0x97)) {
    ...
    private void sendQueryResult() {
        if (s_TradeState == STATE_IDEL) {
            Log.e(TAG, "空闲中,未开始查询");
            s_TradeCode = 0x97;
            SerialFunc.showErrorCode(0xB2, s_TradeCode);
            return;
        }
        if (s_TradeState == STATE_QUERYING) {
            //查询中
            s_TradeCode = 0x01;
            SerialFunc.showErrorCode(0xB2, s_TradeCode);
            Log.e(TAG, "查询中");
        ...
        if (s_TradeState == STATE_QUERY_FINISHED) {
            if (s_TradeCode == 0x00) {
    ...
    private void sendPayResult(long ordernum) {
        if (s_TradeState != STATE_PAY_FINISHED) {
            if (s_TradeState == STATE_IDEL) {
                if (lastPayResult != null) {
                    ...
                    //空闲状态,没有支付过
                    s_TradeCode = 0x97;
                    SerialFunc.showErrorCode(0xC2, s_TradeCode);
                }
            } else if (s_TradeState == STATE_PAYING) {
                //发起支付请求还未完成支付状态
                s_TradeCode = 0x01;
    ...
    private void sendDispelResult() {
        if (s_TradeState != STATE_DISPEL_FINISHED) {
    ...
    public static boolean isEnableScan() {
        return (g_LocalInfo.cDockposFlag == 1
                && (s_TradeState == STATE_PAYING || s_TradeState == STATE_QUERYING));
    }
    ...
    public static void setPayMoney() {
        if (s_TradeState == STATE_PAYING && lngPaymentMoney >= 0) {
            g_WorkInfo.lngPaymentMoney = lngPaymentMoney;
    ...
    public static void clearState() {
        ...
        if (s_TradeState == STATE_DISPELING || s_TradeState == STATE_DISPEL_FINISHED) {
            if (g_UIRDCardHandler != null) {
                g_UIRDCardHandler.sendEmptyMessage(GOTO_PAYCARD);
            }
            s_TradeState = STATE_IDEL;
        }
    }

    public static int getS_TradeState() {
        return s_TradeState;
    }
    ...
    public static void onQueryDone(int queryWay) {
        ...
        s_TradeCode = 0x00;
        s_TradeState = STATE_QUERY_FINISHED;
    }
    ...
    public static void onPayDone(int payResult) {
        ...
        s_TradeState = STATE_PAY_FINISHED;
    }


    public static void setCode(int tradeCode) {
        SerialWorkTask.s_TradeCode = tradeCode;
    }
...
//                }
                break;
            case FINISH_QR_SCAN:
                if (isQRscan && s_TradeState != STATE_PAYING) {
                    Log.e(TAG, "结束识读");
                    g_Nlib.QR_SetDeviceReadEnable(2);//结束识读
                    g_UICardHandler.sendEmptyMessage(HIDE_CAMERAVIEW);
                    isQRscan = false;
                }
                break;
        }
